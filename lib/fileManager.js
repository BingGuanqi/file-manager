...
FileManager.decompress = function *(p) {
  var dir = p.substring(0, p.lastIndexOf("/"))
  var compressMethod = null
  if (p.endsWith(".zip")) {
      compressMethod = compressing.zip
  } else if (p.endsWith(".tar")) {
      compressMethod = compressing.tar
  } else if (p.endsWith(".tgz") || p.endsWith(".tar.gz")) {
      compressMethod = compressing.tgz
  } else if (p.endsWith(".gzip")) {
      compressMethod = compressing.gzip;
  } else {
      throw new Error("unsupported file type")
  }

  yield compressMethod.uncompress(p, dir).catch(err => {
    throw new Error('The true type of the file is not consistent with the suffix name');
  });
}
