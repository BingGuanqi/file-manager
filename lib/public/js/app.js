...
FMApp.controller('FileManagerCtr', ['$scope', '$http', '$location',
  function ($scope, $http, $location) {
    ...
    var setCurFiles = function (relPath) {
      $http.get('api' + relPath)
        .success(function (data) {
          ...
          files.forEach(function (file) {
            var regs = /^[.]$/;
            var char = file.name.slice(0,1);
            if (!regs.test(char)) {
              filterFiles.push(file);
            }
          });

          FM.curFiles = filterFiles;
    ...
    FM.decompress = function () {
      for (var i in FM.selection) {
        console.log(i);
        var relPath = FM.selection[i].relPath;
        var url = 'api/decompress' + relPath;
        console.log(url)
        httpRequest('POST', url, null, null);
      }
    };
